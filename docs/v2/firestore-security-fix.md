...
3. Using an authorized Admin SDK/operator process, provision `is_super_admin: true` only to explicitly approved Firebase Auth UIDs, preserving unrelated claims. No browser API for this is shipped. Confirm recovery access, then obtain a fresh login/token and verify the claim. Use `npm run operator -- revoke-operator` (see [activation runbook](activation-runbook.md)) to revoke: it records `operator_revocations/{uid}`, and the rules reject any claimed token whose `auth_time` precedes that revocation, so already-issued tokens lose global authority immediately.
